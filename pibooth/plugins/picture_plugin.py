...
class PicturePlugin(object):
    ...
    def state_processing_do(self, cfg, app):
        ...
        photo_format = cfg.get('PICTURE', 'photo_format')
        if photo_format == 'strip':
            paper_format = (2, 6)
        else:
            paper_format = (4, 6)

        default_factory = get_picture_factory(captures, cfg.get('PICTURE', 'orientation'), paper_format=paper_format)
        ...
        for savedir in cfg.gettuple('GENERAL', 'directory', 'path'):
            ...
            if photo_format == 'strip':
                # Duplicate strip side-by-side for 4x6 print file
                strip_img = app.previous_picture
                full_width = strip_img.width * 2
                final = Image.new('RGB', (full_width, strip_img.height))
                final.paste(strip_img, (0, 0))
                final.paste(strip_img, (strip_img.width, 0))
                final.save(app.previous_picture_file)
                LOGGER.info("Strip mode: saved doubled 4x6 print file '%s'", app.previous_picture_file)
        ...
        if cfg.getboolean('WINDOW', 'animate') and app.capture_nbr > 1:
            ...
            for capture in captures:
                default_factory = get_picture_factory((capture,), cfg.get(
                    'PICTURE', 'orientation'), paper_format=paper_format, force_pil=True, dpi=200)
                factory = self._pm.hook.pibooth_setup_picture_factory(cfg=cfg,
                                                                      opt_index=idx,
                                                                      factory=default_factory)
